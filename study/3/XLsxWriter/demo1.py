...
def chart_series(cur_row):
    ...
    chart.add_series({
        'categories': '=Sheet1!$B$1:$H$1',   # 将“星期一至星期日”作为图表数据标签(X轴)(x 轴上标出的数字)
        'values':     '=Sheet1!$B$'+cur_row+':$H$'+cur_row,  # 每个频道一周所有数据作为数据区域(每个频道要展出那几天的数据)
        'line':       {'color': 'black'},   # 线条颜色定义为black(黑色)(线条框框的颜色)
        'name':	'=Sheet1!$A$'+cur_row,   # 引用业务名称为图例项 (最右边的频道名称)
    })
